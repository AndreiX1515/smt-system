function jw_select() {
  ...
  document.querySelectorAll(".select").forEach((nativeSel) => {
    ...
    Array.from(nativeSel.options).forEach((o) => {
      ...
      li.innerHTML = o.dataset?.icon
        ? `<i class="${o.dataset.icon}"></i> ${o.textContent}`
        : o.textContent;
    ...
    const choose = (itemEl) => {
      ...
      list
        .querySelectorAll('.jw-select-item[aria-selected="true"]')
    ...
    list.addEventListener("click", (e) => {
      const item = e.target.closest(".jw-select-item");
      if (!item) return;
      e.stopPropagation();
      choose(item);
    });
...
function getCookie(name) {
  ...
  return seg.length === 2
    ? decodeURIComponent(seg.pop().split(";").shift())
    : null;
}
...
function getLangText(el, lang) {
  ...
  if (lang === "tl")
    return el.getAttribute("data-lan-tl") || el.getAttribute("data-lan-eng");
  ...
  try {
    ...
    const isI18nUiEl = (el) => {
      try {
        ...
        return !!el.closest(
          "[data-lan-eng],[data-lan-tl],[data-lan-ko],[data-lan-kor],[data-i18n],[data-i18n-placeholder],[data-i18n-title],[data-i18n-alt]",
        );
      } catch (_) {
        return false;
      }
    ...
    try {
      ...
      for (const n of nodes) {
        ...
        try {
          ...
          if (p && p.closest && p.closest('[data-skip-hangul-scrub="1"]'))
            continue;
        } catch (_) {}
    ...
    try {
      ...
      root.querySelectorAll("*").forEach((el) => {
        ...
        // Do not scrub content explicitly marked as "skip" (DB content areas)
        try {
          if (el.closest && el.closest('[data-skip-hangul-scrub="1"]')) return;
        } catch (_) {}
        ...
        for (const a of attrs) {
          ...
          if (a === "alt") el.setAttribute(a, "Image");
            ...
            el.setAttribute(a, "Please enter a value.");
          else {
            const kind =
              a === "placeholder"
                ? "placeholder"
...
function language_apply(lang) {
  lang = String(lang || "").toLowerCase();
  if (lang !== "eng" && lang !== "tl" && lang !== "kor" && lang !== "ko")
    lang = "eng";
  if (lang === "ko") lang = "kor"; // normalize
  document
    .querySelectorAll(
      "[data-lan-eng],[data-lan-tl],[data-lan-kor],[data-lan-ko]",
    )
    .forEach((el) => {
  ...
  document.querySelectorAll("[data-lan-eng-placeholder]").forEach((el) => {
    ...
    const placeholderKor =
      el.getAttribute("data-lan-kor-placeholder") ||
      el.getAttribute("data-lan-ko-placeholder");
    if (lang === "eng" && placeholderEng) {
      el.setAttribute("placeholder", placeholderEng);
    } else if (lang === "tl") {
      el.setAttribute(
        "placeholder",
        placeholderTl || placeholderEng || el.getAttribute("placeholder") || "",
      );
  ...
  document.querySelectorAll("[data-lan-eng-alt]").forEach((el) => {
    ...
    const altKor =
      el.getAttribute("data-lan-kor-alt") || el.getAttribute("data-lan-ko-alt");
    if (lang === "eng" && altEng) {
  ...
  document.querySelectorAll("select option[data-lan-eng]").forEach((option) => {
    ...
    const txtKor =
      option.getAttribute("data-lan-kor") || option.getAttribute("data-lan-ko");
    if (lang === "eng" && txtEng) {
      ...
      option.textContent =
        txtKor || option.getAttribute("data-lan-eng") || option.textContent;
    }
  });
...
window.refreshAllJwSelect = function refreshAllJwSelect() {
  try {
    document.querySelectorAll(".jw-select").forEach((wrap) => {
      ...
      // selected 박스 텍스트
      const opt =
        nativeSel.selectedOptions?.[0] || nativeSel.options?.[0] || null;
...
(function __admin_i18n_bootstrap() {
  try {
    ...
    try {
      __admin_scrub_hangul(document.body);
    } catch (_) {}

    // show
    try {
      root.setAttribute("data-admin-i18n-ready", "1");
    } catch (_) {}
  } catch (_) {
    try {
...
/* ══════════════════════════════════════════════════════
   ...
   · Only sets max-width — never a fixed height
   · Adds data-modal-id for scoped close targeting
   · Adds 'dialog--ready' class after content loads (CSS animates it)
   · Everything else (fetch, script exec, events) unchanged
   ══════════════════════════════════════════════════════ */

class Modal {
  constructor(action, width, sq) {
    this.action = action;
    this.width = width; // max-width only — height is always auto
    this.sq = sq;
    this.el = null;
    this._abort = null;
  }
  ...
  static _execScripts(container) {
    try {
      ...
      console.error("[Modal] script exec failed:", e);
    }
  }
  ...
  async open() {
    ...
    /* Width only — let height be content-driven */
    if (this.width) {
    ...
    d.innerHTML = `<div class="dialog-loading">
    ...
    /* showModal() gives us ::backdrop + native centering for free */
    typeof d.showModal === "function"
      ? d.showModal()
      : d.setAttribute("open", "");
    ...
    /* Close on Escape (native behavior, but also fires our cleanup) */
    ...
    if (this.action) {
      try {
        ...
        d.innerHTML = html;
        d.classList.add("dialog--ready"); /* triggers CSS enter animation */
        ...
        /* Wire close button from loaded HTML */
        d.querySelector("#closeDialog")?.addEventListener(
          "click",
          () => this.close(),
          { once: true },
        );
  ...
  async update(action, sq) {
    ...
    try {
      ...
      document.dispatchEvent(
        new CustomEvent("modal:loaded", {
          detail: { dialog: this.el, action: url, data },
        }),
      );
    } catch (err) {
...
function modal_close() {
  ...
  last.addEventListener(
    "animationend",
    () => {
      typeof last.close === "function" && last.close();
      last.remove();
    },
    { once: true },
  );
...
function member_info(btn, page) {
  ...
  if (!wrap.innerHTML.trim()) {
    wrap.innerHTML =
      '<div style="padding:10px;font-size:13px;color:#6b7280;">Loading...</div>';
    fetch(page + "?" + Date.now(), { cache: "no-store" })
      ...
      .catch(() => {
        wrap.innerHTML =
          '<div style="padding:10px;font-size:13px;color:#ef4444;">Failed to load.</div>';
        openBox();
      });
  } else {
    openBox();
...
function initLogoutButton(container) {
  const logoutBtn = container
    ? container.querySelector("#logoutBtn")
  ...
  const changePasswordBtn = container
    ? container.querySelector("#changePasswordBtn")
    : document.getElementById("changePasswordBtn");
  if (
    changePasswordBtn &&
    !changePasswordBtn.hasAttribute("data-change-password-initialized")
  ) {
...
(function bindChangePasswordDelegatedOnce() {
  try {
    ...
    document.addEventListener(
      ...
      function (e) {
        try {
          console.log("[ADMIN] changePassword click captured:", e && e.target);
        } catch (_) {}
        const t =
          e.target && e.target.closest
            ? e.target.closest("#changePasswordBtn")
            : null;
        if (!t) return;
        try {
  ...
  try {
    ...
    if (headerName) {
      ...
      if (ut === "admin") headerName.textContent = "Admin";
        ...
        headerName.textContent = data.displayName || "Agent";
      else if (ut === "guide")
        headerName.textContent = data.displayName || "Guide";
...
async function handleLogout(e) {
  ...
  try {
    ...
    const apiPath = "../../public/backend/api/logout.php";
...
function layerToggle(btn) {
  const menu = btn
    .closest(".layerToggleWrap")
    ?.querySelector(":scope > .fab-menu");
  if (!menu) return;
...
function togglePlusMinus(btn) {
  ...
  var targets = target
    ? document.querySelectorAll('[data-section-name="' + target + '"]')
    : [];
  ...
  if (hasOn) {
    ...
    targets.forEach(function (el) {
      if (el !== btn) el.classList.remove("hidden");
    });
    ...
    targets.forEach(function (el) {
      if (el !== btn) el.classList.add("hidden");
    });
...
function scrollToSection(sectionId) {
  ...
  if (!target) {
    ...
    target = document.querySelector(
      'h2.section-title[data-lan-eng="' + sectionId + '"]',
    );
  }
  if (target) {
    // 섹션이 숨겨져 있으면 먼저 표시
    if (target.classList.contains("hidden")) {
      var btn = document.querySelector(
        'button.aside-add-section[data-target-section="' + sectionId + '"]',
      );
...
function toggleLayoutNav() {
  ...
  if (!nav) return;
  // Desktop: collapse/expand | Mobile: drawer open/close
...
async function _loadFragment(url, selector) {
  ...
  try {
    ...
    console.error(`[init] fragment load failed: ${url}`, err);
  }
}

/* ── Layout: script executor ──────────────────────── */
...
const _nav = {
  init() {
    document.querySelectorAll("#layoutNav .nav-btn").forEach((btn) => {
      if (btn.tagName === "BUTTON") {
        btn.addEventListener("click", () => _nav.toggle(btn));
      }
    });
  ...
  toggle(btn) {
    ...
    /* Drive inline style directly — beats any legacy CSS !important */
  ...
  setActive(slug) {
    if (!slug) return;
    document
      .querySelectorAll("#layoutNav .side-link")
      .forEach((a) => a.classList.remove("is-active"));
    document
      .querySelectorAll("#layoutNav .nav-item")
      .forEach((li) => li.classList.remove("is-open", "has-active"));

    let matched = null;

    // sub-menu links
    document.querySelectorAll("#layoutNav .side-link").forEach((a) => {
    ...
    // direct-link nav buttons (e.g. Inquiries)
    document.querySelectorAll("#layoutNav a.nav-btn").forEach((a) => {
      const pages = (a.dataset.page || "").split(",").map((s) => s.trim());
      if (pages.includes(slug)) {
        a.classList.add("is-active");
        matched = a;
    ...
    if (parentItem) {
      ...
      parentItem
        .querySelector("button.nav-btn")
        ?.setAttribute("aria-expanded", "true");

      /* Also set inline style so legacy CSS cannot override the open state */
...
const _router = {
  ...
  start() {
    document.addEventListener("click", (e) => {
      ...
      if (!a || !_router._isInternal(a)) return;
    ...
    window.addEventListener("popstate", () => {
      _router.navigate(location.href, { pushState: false });
    });
  },
  ...
  async _swap(html, href, pushState) {
    ...
    if (newSect.dataset.pageSlug)
      content.dataset.pageSlug = newSect.dataset.pageSlug;
...
/* ── Layout: init (replaces your existing init call) ─ */
