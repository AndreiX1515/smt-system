...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (!session.authenticated || session.userType !== 'agent') {
...
function applyFilters() {
    ...
    currentFilters.travelStartDate = document.getElementById('travelStartDate').value.trim();
    currentPage = 1;
    saveFilters();
    loadBookings();
}
...
function saveFilters() {
    sessionStorage.setItem('travelDocAgentFilters', JSON.stringify({
        ...currentFilters,
...
function restoreFilters() {
    const saved = sessionStorage.getItem('travelDocAgentFilters');
    if (saved) {
        try {
            const f = JSON.parse(saved);
            currentFilters = { search: f.search || '', travelStartDate: f.travelStartDate || '' };
...
function renderTable() {
    ...
    if (pageItems.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="is-center" style="padding:40px;color:#999;">No bookings found</td></tr>';
