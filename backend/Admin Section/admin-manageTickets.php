...
<head>
  ...
  <link rel="stylesheet" href="../Admin Section/assets/css/admin-manageTicket.css?v=<?php echo time(); ?>">
...
<body>
  <div class="body-container">
    ...
    <div class="main-content-container">
      <div class="navbar">
        <h5 class="title-page">Manage Tickets</h5>
      </div>
          ...
          <div class="table-container">
            <table id="product-table" class="product-table">
              ...
              $sql = "SELECT 
                ...
                concernType AS `Concern Type`,
                numUsers AS `Number of Users`,
                ...
                status AS `Status`
            FROM tickets
            ORDER BY createdAt DESC";  // Show most recent tickets first
              ...
              <tbody>
                ...
                if ($result->num_rows > 0) {
                  while ($row = $result->fetch_assoc()) {
                    $ticketId = htmlspecialchars($row['Ticket ID']);

                    echo "<tr>
                    <td>{$row['Ticket ID']}</td>
                    <td>{$row['Agent ID']}</td>
                    <td>{$row['Agent Code']}</td>
                    <td>{$row['Concern Type']}</td>
                    <td>" . ($row['Number of Users'] ?? '') . "</td>
                    ...
                    <td>
                        <div class='dropdown-center' style='text-align: center; position: relative;'>
                            ...
                            <ul class='dropdown-menu' style='position: absolute; bottom: 100%; left: 50%; transform: translateX(-50%);'>
                                <li>
                                    <a class='dropdown-item edit' href='#' data-id='{$ticketId}' data-bs-toggle='modal' data-bs-target='#editModal'>
                                        <i class='fas fa-edit'></i> Edit
                                    </a>
                                </li>
                                <li>
                                    <a class='dropdown-item delete text-danger' href='#' data-id='{$ticketId}' data-bs-toggle='modal' data-bs-target='#deleteModal'>
                  ...
                  echo "<tr><td colspan='10' style='text-align: center;'>No ticket records found</td></tr>";
                }
                ?>
              </tbody>
            </table>
